...
class CNStimProfile : public INStimProfile
{
	...
	double m_ScreenSizeX;		// screen dimension in cm
	double m_ScreenSizeY;	
	double m_ScreenDistance;	// distance away in cm

	bool m_bShouldHideCursor;

	char m_LogPath[512];

	static CNStimProfile* m_pStimProfile;
public:
	CNStimProfile();
	virtual ~CNStimProfile();

	virtual HRESULT __stdcall GetLogPath(unsigned char* pLogPath) {
		strcpy((char*)pLogPath, m_LogPath);
		return S_OK;
	}
	...
	virtual HRESULT __stdcall SetDefault() {
		...
		m_ScreenSizeY = 10;
		m_ScreenDistance = 48;
		m_bShouldHideCursor = false;

		strcpy(m_LogPath, "c:\\temp");
		CreateDirectory(m_LogPath, 0);
	...
	virtual HRESULT __stdcall Load() {
		...
		m_bShouldHideCursor = m_Profile.ReadInt("SCREEN","Should Hide Cursor") ? true : false;
		m_ScreenSizeX = m_Profile.ReadDouble("SCREEN","ScreenWidth");
		m_ScreenSizeY = m_Profile.ReadDouble("SCREEN","ScreenHeight");
		m_ScreenDistance = m_Profile.ReadDouble("SCREEN","ScreenDistance");
		
		strcpy(m_LogPath, m_Profile.ReadString("CONFIG", "LogPath"));
		CreateDirectory(m_LogPath, 0);
		return S_OK;
	}
	...
	virtual HRESULT __stdcall Save() {
		...
		m_Profile.WriteInt("SCREEN","Should Hide Cursor", m_bShouldHideCursor);

		m_Profile.WriteInt("SCREEN", "ScreenWidth", m_ScreenSizeX);
		m_Profile.WriteInt("SCREEN", "ScreenHeight", m_ScreenSizeY);
		m_Profile.WriteInt("SCREEN", "ScreenDistance", m_ScreenDistance);

		m_Profile.WriteString("CONFIG", "LogPath", m_LogPath);
		return S_OK;
	}
	...
	virtual HRESULT __stdcall ShouldHideCursor() { return m_bShouldHideCursor?S_OK:S_FALSE; }
...
/*		(*pWidth) = 1920;
	...
	virtual double __stdcall GetConversionFactor(const char* type);
	virtual double __stdcall ConvertUnit(double src, const char* srcStr, const char* destStr);
	virtual ULONG __stdcall AddRef(void);
	virtual HRESULT __stdcall QueryInterface(REFIID iid, void **ppvObject);
	virtual ULONG __stdcall Release(void);

	static CNStimProfile* GetStimProfile();
};
