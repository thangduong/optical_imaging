...
void CNStimSequence::Compile(INStimEngine* pEngine)
{
	...
	CList<NStimSequenceMarker*,NStimSequenceMarker*> TallyList;
	...
	while (pos)
	{
		...
		case SM_START:
			{
			POSITION tlPos = TallyList.GetHeadPosition();
			POSITION lmAddPos = 0;
			while (tlPos) {
				POSITION lastPos = tlPos;
				if (TallyList.GetNext(tlPos)->iOrder >= lastMark->iOrder) {
					lmAddPos = lastPos;
					break;
				}
			}
			if (lmAddPos) {
				TallyList.InsertBefore(lmAddPos, lastMark);
			}
			else
				TallyList.AddTail(lastMark);
...
//					if (TallyList.GetNext(cpos)->IsAtom((unsigned char*)lastMark->AtomName.GetData()) == S_OK)
					if (strcmpi(TallyList.GetNext(cpos)->AtomName.GetData(), lastMark->AtomName.GetData())==0)
					{
		...
		if (lastMark->iFrameMarker != nextMark->iFrameMarker)
		{
			...
			{
				pAtom = 0;
				pEngine->FindAtom((unsigned char*)TallyList.GetNext(cpos)->AtomName.GetData(), &pAtom);
				if (pAtom == 0) {
					DebugBreak();
				}
				pCompiledMarker->AddAtom(pAtom);
			}
...
//				pCompiledMarker->AddAtom(TallyList.RemoveHead());
	...
	{
		pAtom = 0;
		pEngine->FindAtom((unsigned char*)TallyList.RemoveHead()->AtomName.GetData(), &pAtom);
		if (pAtom == 0) {
			DebugBreak();
		}
		pCompiledMarker->AddAtom(pAtom);
...
void CNStimSequence::GetAtomList(int frame, CList<INStimAtom*,INStimAtom*>** ppList, INStimEngine* pEngine)
{
	if (!m_bSequenceDone) {
