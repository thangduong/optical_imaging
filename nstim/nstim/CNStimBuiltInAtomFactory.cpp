...
HRESULT __stdcall CNStimBuiltInAtomFactory::CreateAtom(unsigned char* AtomType, INStimAtom** ppAtom)
{
	...
	else if (strcmpi((const char*)AtomType, "smgrating") == 0)
	{
		if (ppAtom)
			(*ppAtom) = new CNStimSMGratingPatch;
		hResult = S_OK;
	}
	else if (strcmpi((const char*)AtomType, "oigrating") == 0)
	{
		if (ppAtom)
			(*ppAtom) = new CNStimOIGratingPatch;
		hResult = S_OK;
	}
	else if (strcmpi((const char*)AtomType, "binarynoise") == 0)
	{
	...
	else if (strcmpi((const char*)AtomType, "leech") == 0)
	{
		if (ppAtom)
			(*ppAtom) = new CNStimLeechAtom;
		hResult = S_OK;
	}
	else if (strcmpi((const char*)AtomType, "gammaelt") == 0)
	{
		if (ppAtom)
			(*ppAtom) = new CNStimAtomGammaElt;
		hResult = S_OK;
	}
	return hResult;
}
