// CNStimSMGratingPatch.cpp: implementation of the CNStimSMGratingPatch class.
//
//////////////////////////////////////////////////////////////////////

#include "CNStimSMGratingPatch.h"
#include "CDirect3DDevice.h"
#include "CNStimFloatPatchParam.h"
#include <stdio.h>
#include "CNStimEngine.h"

char* CNStimSMGratingPatch::m_pPatchType = "grating";
float* CNStimSMGratingPatch::sineTable = 0;
float* CNStimSMGratingPatch::cosineTable = 0;
bool CNStimSMGratingPatch::lutInitialized = false;
...
CNStimSMGratingPatch::CNStimSMGratingPatch()
{
	if (!lutInitialized) {
...
HRESULT __stdcall CNStimSMGratingPatch::AdvanceFrame(int numFrames)
{
	if ((*m_pBiDir))
	{
...
HRESULT __stdcall CNStimSMGratingPatch::Draw(INStimEngine* pEngine)
{
	...
	m_GratingBuff = Device.CreateTexture(m_iGratingBuffWidth, m_iGratingBuffHeight,0,D3DUSAGE_AUTOGENMIPMAP,D3DFMT_A8R8G8B8,D3DPOOL_DEFAULT);
	if (!m_GratingBuff.IsNull()) {
		IDirect3DSurface9* pds;
		m_GratingBuff->GetSurfaceLevel(0,&pds);
		HRESULT hr= Device->UpdateSurface(m_GratingSurf.GetDirect3DSurface(), 0, pds, 0);
		pds->Release();
	}
	else
		MessageBeep(-1);
	m_BackBuff = Device.CreateTexture(m_iBackBuffWidth, m_iBackBuffHeight,0,D3DUSAGE_AUTOGENMIPMAP|D3DUSAGE_RENDERTARGET,D3DFMT_A8R8G8B8,D3DPOOL_DEFAULT);
	if (!m_BackBuff.IsNull()) {
		IDirect3DSurface9* pds;
		m_BackBuff->GetSurfaceLevel(0,&pds);
		HRESULT hr= Device->UpdateSurface(m_BackSurf.GetDirect3DSurface(), 0, pds, 0);
		pds->Release();
	}
	else
		MessageBeep(-1);
	
	int iFrameShift = ((int)(((m_iCurrentFrame%m_iNumFrames) * (*m_pSfPixPerCycle) / m_iNumFrames) + ((*m_pSpatialPhase)*(*m_pSfPixPerCycle))/360))%(*m_pSfPixPerCycle);
...
HRESULT __stdcall CNStimSMGratingPatch::Init(INStimEngine* pEngine)
{
	...
	if (m_pDevice)
	{
		...
		m_GratingSurf = Device.CreateSurface(m_iGratingBuffWidth,m_iGratingBuffHeight,D3DFMT_A8R8G8B8,D3DPOOL_SYSTEMMEM);
		m_GratingSurf->LockRect(&lr,0,0);
		temp = (char*)lr.pBits;
...
/*				double sine = sin((double)x * 2 * pi / (double)(*m_pSfPixPerCycle));
				double distance = x - (m_iGratingBuffWidth)/2;
				double sineAngle = (double)distance * 2 * pi / (double)(*m_pSfPixPerCycle);
				int sineIdx = (int)((sineAngle*180/pi)*LUT_RESOLUTION);
				if (sineIdx < 0)
				{
					int N = (sineIdx / (LUT_RESOLUTION*360)) + 1;
					sineIdx = sineIdx + LUT_RESOLUTION*360*N;
				}
				*/
		...
		m_BackSurf = Device.CreateSurface(m_iBackBuffWidth,m_iBackBuffHeight,D3DFMT_A8R8G8B8,D3DPOOL_SYSTEMMEM);
		m_BackSurf->LockRect(&lr,0,0);
		temp = (char*)lr.pBits;
...
HRESULT __stdcall CNStimSMGratingPatch::Reset(INStimEngine* pEngine)  {
...
HRESULT __stdcall CNStimSMGratingPatch::SetParam(unsigned char* pParamName, unsigned char* pParamValue) {
		...
		(strcmpi((char*)pParamName, "SpatialFrequency") == 0) )
	{
		...
		if (m_pDevice && m_pEngine)
		{
			...
			if ((*m_pTfCycPerSec) > 0) {
				double framerate;
				m_pEngine->GetFrameRate(&framerate);
				m_iNumFrames = framerate / (*m_pTfCycPerSec);
			}
			else {
				m_iNumFrames = 1;
