...
void CModuleDlg::OnSelchangeModuleList() 
{
	...
	sscanf(m_fullModName,"ITI.%s.%01d",m_strModName,&m_modSeqNum);
}
...
void CModuleDlg::OnOK() 
{
	...
	if (m_vidSrcCfg->CreateVideoSrc(m_curNode, m_fullModName, 1,&parm,&m_pICap) != S_OK) {
		...
		if ( camAttrib.dwBitsPerPixel != 8 ) {
			::MessageBox(NULL,"Unable to use selected image capture module because color is not 8 bit Mono.",
			"Video Source must be 8 bit Mono",MB_OK);
