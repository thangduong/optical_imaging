...
Public Sub Invert(ByRef h_Buffer() As Long, ringSeqNum As Long, Image_DX As Long, Image_DY As Long, SrcBytesPP As Long)

    Dim x As Long
    Dim startX As Long
    startX = ringSeqNum * Image_DX * Image_DY * SrcBytesPP / 4
    For x = startX To startX + (Image_DX * Image_DY * SrcBytesPP / 4) - 1
            h_Buffer(x) = h_Buffer(x) Xor &HFFFFFFFF
    Next x
End Sub
