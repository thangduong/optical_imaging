...
CTransform::CTransform(CWnd* pParent /*=NULL*/,CICamera *cam)
...
BOOL CTransform::OnInitDialog() 
{
	...
	if (!m_cam->TransformSupported(IFC_FLIP_TRANSFORM))
		m_ctlFlip.EnableWindow(FALSE);

	if (!m_cam->TransformSupported(IFC_MIRROR_TRANSFORM))
		m_ctlMirror.EnableWindow(FALSE);

	if (!m_cam->TransformSupported(IFC_ROT180_TRANSFORM))
