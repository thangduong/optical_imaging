...
//
//  FILE:           IFC_PC2C_ThreadProcDlg.cpp
//
...
//              Constructor
//------------------------------------------
CIFC_PC2C_ThreadProcDlg::CIFC_PC2C_ThreadProcDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CIFC_PC2C_ThreadProcDlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(CIFC_PC2C_ThreadProcDlg)
	//}}AFX_DATA_INIT
...
//              DoDataExchange
//------------------------------------------
void CIFC_PC2C_ThreadProcDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CIFC_PC2C_ThreadProcDlg)
...
BEGIN_MESSAGE_MAP(CIFC_PC2C_ThreadProcDlg, CDialog)
	//{{AFX_MSG_MAP(CIFC_PC2C_ThreadProcDlg)
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
...
//              OnInitDialog
//------------------------------------------
BOOL CIFC_PC2C_ThreadProcDlg::OnInitDialog()
{
...
//              InitIFC_Capture
//------------------------------------------
BOOL CIFC_PC2C_ThreadProcDlg::InitIFC_Capture()
{
CAM_ATTR attrib;
    ...
    if (m_pBrdMod == NULL)
    {
        // Failed to find and use the default configuration file "cl2test.txt"
        // Ask user to select a camera type

        m_pBrdMod = IfxCreateCaptureModule("CL2", 0);       // Use PC2-Camlink board # 0
        if (m_pBrdMod == NULL)
        {
            AfxMessageBox(_T("Can't find PC2Camlink board in the system !!!"));
            return FALSE;
        }
...
//              InitIFC_Callback
//------------------------------------------
BOOL CIFC_PC2C_ThreadProcDlg::InitIFC_Callback()
{

    // Want a callback on the SKIPPED FIELD

    m_pIntCallback_SKF = m_pBrdMod->CreateIntrCallback(CL2_INTR_FRAME_SKIPPED, MyCallback_SKF, this);
...
//              ReleaseIFC_Callback
//------------------------------------------
void CIFC_PC2C_ThreadProcDlg::ReleaseIFC_Callback()
{
...
//------------------------------------------
BOOL CIFC_PC2C_ThreadProcDlg::AllocateCaptureMemory()
{
DWORD dwAllocSize;
...
//------------------------------------------
void CIFC_PC2C_ThreadProcDlg::ReleaseCaptureMemory()
{
...
//------------------------------------------
CImgConn *CIFC_PC2C_ThreadProcDlg::InitIFC_ImageConnection(HWND hWnd, CICamera *cam)
{
CImgConn *ImgConn;
    ...
    if (ImgConn)
    {
		if (m_dwPixBitDepth > 8 && m_dwPixBitDepth < 16)
		{
...
//         ReleaseIFC_Capture
//------------------------------------------
void CIFC_PC2C_ThreadProcDlg::ReleaseIFC_Capture()
{
...
//------------------------------------------
void CIFC_PC2C_ThreadProcDlg::ReleaseIFC_ImageConnection()
{
...
//------------------------------------------
void CIFC_PC2C_ThreadProcDlg::OnPaint() 
{
	if (IsIconic())
	{
...
//------------------------------------------
HCURSOR CIFC_PC2C_ThreadProcDlg::OnQueryDragIcon()
{
...
//              ReleaseAll
//------------------------------------------
void CIFC_PC2C_ThreadProcDlg::ReleaseAll()
{
...
//              SetPortAndCamType
//------------------------------------------
void CIFC_PC2C_ThreadProcDlg::SetPortAndCamType(char *pszCamType)
{
CAM_ATTR attr;
CString strToShow;
    ...
    m_dwBytePerPixel    = attr.dwBytesPerPixel;
    m_CamColor          = attr.color;
    m_dwImageSize       = m_dwCamSizeX * m_dwCamSizeY * m_dwBytePerPixel;
...
//              ClearDisplay
//------------------------------------------
void CIFC_PC2C_ThreadProcDlg::ClearDisplay()
{
CClientDC MyDC(this);
CBrush *pOldBrush;
RECT RectDraw;
...
//------------------------------------------
BOOL CIFC_PC2C_ThreadProcDlg::GetWindowPosition(HWND hWnd, LPRECT lpRect)
{
RECT  WinPosition;
...
//              ShowStatistics
//------------------------------------------
void CIFC_PC2C_ThreadProcDlg::ShowStatistics() 
{
float fRate;
CString strToShow;
...
//              UpdateButtons
//------------------------------------------
void CIFC_PC2C_ThreadProcDlg::UpdateButtons()
{
...
// the thread of this dialog box class (CIFC_PC2C_ThreadProcDlg).
//
...
//------------------------------------------
void CIFC_PC2C_ThreadProcDlg::ThreadAcqFunc(void *pContext) 
{
CIFC_PC2C_ThreadProcDlg *pMyClass;
CString strToShow;
int iSeqNum;
...
DWORD dwErrorCode;
    ...
    pMyClass = (CIFC_PC2C_ThreadProcDlg *)pContext;   // Context is the ptr to my class
    ...
    while (pMyClass->m_bIsGrabbing)
    {
        ...
        else if (iSeqNum != -1)
        {
            ...
            // Determine if another frame is to be capture
...
// event occurs
//------------------------------------------
void CIFC_PC2C_ThreadProcDlg::MyCallback_SKF(void *pContext)
{
CIFC_PC2C_ThreadProcDlg *pMyClass;
    ...
    pMyClass = (CIFC_PC2C_ThreadProcDlg *)pContext;
...
//------------------------------------------
void CIFC_PC2C_ThreadProcDlg::ThreadProcFunc(void *pContext) 
{
CIFC_PC2C_ThreadProcDlg *pMyClass;
...
int iBufferCountPending;
    ...
    while (iBufferCountPending > 0)
    {
        ...
        if (pMyClass->m_dwPixBitDepth == 8)
        {
            pMyClass->ProcImageFunctionMono8(pTheProcThreadParams->pTheBuffer, pTheProcThreadParams->iBufferIndex,
                                pMyClass->m_dwCamSizeX, pMyClass->m_dwCamSizeY);
        }
        else
        {
            pMyClass->ProcImageFunctionMono10(pTheProcThreadParams->pTheBuffer, pTheProcThreadParams->iBufferIndex,
                                pMyClass->m_dwCamSizeX, pMyClass->m_dwCamSizeY);
        }
...
//------------------------------------------
//              ProcImageFunctionMono8
//------------------------------------------
// Apply a thresholding, then draw a white
// block positioned according to buffer index.
//------------------------------------------
void CIFC_PC2C_ThreadProcDlg::ProcImageFunctionMono8(BYTE *pImageBuffer, DWORD dwBufferIndex,
...
//------------------------------------------
//              ProcImageFunctionMono10
//------------------------------------------
// Apply a thresholding, then draw a white
// block positioned according to buffer index.
//------------------------------------------
void CIFC_PC2C_ThreadProcDlg::ProcImageFunctionMono10(BYTE *pImageBuffer, DWORD dwBufferIndex,
                                                       DWORD dwImageSizeX, DWORD dwImageSizeY)
{
WORD *pHostBufferProc;
DWORD nLine;
DWORD nPixel;
DWORD dwLinePitch;
    ...
    pHostBufferProc = (WORD *)pImageBuffer;
    dwLinePitch = dwImageSizeX * m_dwBytePerPixel;

    for (nLine = 0; nLine < dwImageSizeY; nLine++)
    {
        for (nPixel = 0; nPixel < dwImageSizeX; nPixel++)
        {
            if (*pHostBufferProc < 300)
                *pHostBufferProc = 0;
            else if (*pHostBufferProc > 700)
                *pHostBufferProc = 0xFFFF;
    ...
    if (((30 * dwBufferIndex) + 30) < dwImageSizeY)     // Don't go beyond image floor
    {
        ...
        {
            pHostBufferProc = (WORD *)pImageBuffer;
            pHostBufferProc += (30 * dwLinePitch * dwBufferIndex);
            pHostBufferProc += (nLine * dwLinePitch);
...
//  view window.
//------------------------------------------
void CIFC_PC2C_ThreadProcDlg::DrawWindowBorder(HWND hWnd)
{
CClientDC MyDC(this);
...
//          DrawRectangleLight()
//------------------------------------------
void CIFC_PC2C_ThreadProcDlg::DrawRectangleLight(HWND hWnd, COLORREF TheColor, BOOL bDrawText, CString& strTheText)
{
CClientDC MyDC(this);
...
//              OnButtonCam
//------------------------------------------
void CIFC_PC2C_ThreadProcDlg::OnButtonCam() 
{
CCamSelDlg CamDlg(this, m_pCam);
...
//              OnButtonFreeze
//------------------------------------------
void CIFC_PC2C_ThreadProcDlg::OnButtonFreeze() 
{
...
//              OnButtonGrab
//------------------------------------------
void CIFC_PC2C_ThreadProcDlg::OnButtonGrab() 
{
    ...
    m_GrabID = m_pCam->Grab(IFC_GOPT_AUTO_FRAME_LOCK, m_pHostCapMemory,
                             BUFFER_COUNT, m_iFrameCountToGrab);
...
//              OnButtonSnap
//------------------------------------------
void CIFC_PC2C_ThreadProcDlg::OnButtonSnap() 
{
CString strToShow;
...
//              OnClose
//------------------------------------------
void CIFC_PC2C_ThreadProcDlg::OnClose() 
{
CString strWarnText;
...
//              OnTimer
//------------------------------------------
void CIFC_PC2C_ThreadProcDlg::OnTimer(UINT nIDEvent) 
{
...
//              OnButtonTrigger
//------------------------------------------
void CIFC_PC2C_ThreadProcDlg::OnButtonTrigger() 
{
...
//              OnButtonTrap
//------------------------------------------
void CIFC_PC2C_ThreadProcDlg::OnButtonTrap() 
{
