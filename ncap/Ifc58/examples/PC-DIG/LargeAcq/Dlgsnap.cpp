 ...
 *    $Logfile: /ifc/examples/pcdig/LargeAcq/Dlgsnap.cpp $
 *    $Revision: 1.1 $
 *    $Modtime: 10/09/03 10:09a $
 *
 * ABSTRACT
...
void CDlgsnap::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
   ...
   case SB_THUMBTRACK:   // Drag scroll box to specified position. nPos is the
	...
	m_vparent->DisplayImage(m_vparent->m_pHostBuffFrame);
...
void CDlgsnap::OnSnap() 
{
	...
	m_vparent->DisplayImage(m_vparent->m_pImgBuff);
}
...
void CDlgsnap::OnPlay() 
{
	...
	for (i=0;i<m_vparent->m_numFramesInHostBuf;i++) {
		m_slider->SetPos(i);
		frameAddr = m_vparent->m_pImgBuff + i * (m_areaDx*m_areaDy*m_bytesPerPixel);
		m_vparent->DisplayImage(frameAddr);
