 ...
 *    $Logfile: /ifc/examples/pcdig/LargeAcq/LargeAcqView.cpp $
 *    $Revision: 1.1 $
 *    $Modtime: 10/09/03 11:22a $
 *
 * ABSTRACT
...
void CLargeAcqView::OnInitialUpdate()
{
	...
	if (bayerFilter != IFC_BAYER_NONE) {
		m_pDispBuff = (BYTE*)GlobalAllocPtr(GMEM_FIXED,imgDx * imgDy * 4);
		m_hImgSrc = IfxCreateImgSrc( (unsigned char *)m_pDispBuff, (WORD)imgDx, (WORD)imgDy, 32, 
			IFC_RGB,0,0,(WORD)sinkDx,(WORD)sinkDy);
	}
	else {
...
#ifndef IFC_BAYOPT_12BIT_SRC
#define IFC_BAYOPT_12BIT_SRC		8
#endif

void CLargeAcqView::DisplayImage(BYTE *pImage)
{
	CLargeAcqApp *theapp=(CLargeAcqApp*)AfxGetApp();	
	...
	if (bayerFilter != IFC_BAYER_NONE) {
		...
		if (cam->AcqParamSupported(P_BAYER_GAIN_BLUE)) {
			IFC_BAYER_EXT_ATTR extAttr;
			extAttr.blueGain = (float)cam->GetAcqParam(P_BAYER_GAIN_BLUE);
			extAttr.greenGain = (float)cam->GetAcqParam(P_BAYER_GAIN_GREEN);
			extAttr.redGain = (float)cam->GetAcqParam(P_BAYER_GAIN_RED);					
			IfxBayerFilterConvert(bayOptions,bayerFilter,m_pDispBuff, pImage, imgDx, imgDy,IFC_RGB,&extAttr);
		}
		else {
			IfxBayerFilterConvert(bayOptions,bayerFilter,m_pDispBuff, pImage,imgDx, imgDy,IFC_RGB,NULL);
		}
	}
	else {				
		// Update the address associated with the image source to point to the frame
		// we just processed
		m_hImgConn->GetSrc()->SetBufferAddr(pImage);
	}
