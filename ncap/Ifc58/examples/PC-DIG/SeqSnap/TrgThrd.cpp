 ...
 *    $Logfile: /ifc/examples/pcdig/seqsnap/TrgThrd.cpp $
 *    $Revision: 1.1 $
 *    $Modtime: 10/25/03 8:55a $
 *
 * ABSTRACT
...
DWORD WINAPI TriggerThread(LPVOID owner)
{
	...
	DWORD totalTime;
	BYTE *pHostBuffFrame;
	CInterrupt intr_eot(theapp->m_iCap,PCD_INTR_EOACQ);
	...
	for (frame_count=0;!proc->m_shutdown;) {
		...
		if (m_trigProp->m_continuousSWTrig) {
			if (m_trigProp->m_startEvent == PCD_TMODE_SWTRIG0)
				cam->SetAcqParam(P_GEN_SW_TRIGGER,0,0);
			else if (m_trigProp->m_startEvent == PCD_TMODE_SWTRIG1)
