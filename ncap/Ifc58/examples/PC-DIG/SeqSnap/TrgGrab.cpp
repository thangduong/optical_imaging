...
: CPropertyPage(CTrgGrab::IDD)
{
	...
	m_minTime = 0;
	m_avgTime = 0;
	m_startEvent = PCD_TMODE_DISABLE;
	m_stopEvent = PCD_TMODE_DISABLE;
	m_trgThread = NULL;
...
CTrgGrab::~CTrgGrab()
{
	...
	if (m_trgThread)
		delete m_trgThread;
	((CPCDig*)(theapp->m_iCap))->SetAcqTriggerEvents(PCD_TMODE_DISABLE ,PCD_TMODE_DISABLE);
}
...
void CTrgGrab::OnTrgstart() 
{
	...
	if (m_TrigStartEvent != 0)
		m_startEvent = (PCD_TRIG_MODE_TYPE)(m_TrigStartEvent+1);
	else 
		m_startEvent = PCD_TMODE_DISABLE;

	if (m_TrigEndEvent != 0)
		m_stopEvent = (PCD_TRIG_MODE_TYPE)(m_TrigEndEvent+1);
	else 
		m_stopEvent = PCD_TMODE_DISABLE;

	((CPCDig*)(theapp->m_iCap))->SetAcqTriggerEvents(m_startEvent ,m_stopEvent);
