 ...
 *		$Logfile: /ifc/examples/pcdig/intrex/INTREX.CPP $
 *		$Revision: 1.1 $
 *		$Modtime: 6/10/99 4:50p $
 *
 * ABSTRACT  
...
// boards and will appear in all applications using this hardware
	...
	for (m_iBoardCount = 0;m_iBoardCount < MAX_BOARD_COUNT ;m_iBoardCount++) {
		...
		if (!(m_pBrdMod[m_iBoardCount] = IfxCreateCaptureModule("PCD", m_iBoardCount,"pcdtest.txt"))) {
			m_pBrdMod[m_iBoardCount] = IfxCreateCaptureModule("PCD", m_iBoardCount);
