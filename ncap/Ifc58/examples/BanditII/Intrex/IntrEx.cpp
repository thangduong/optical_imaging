 ...
 *    $Logfile: /ifc/examples/BanditII/Intrex/IntrEx.cpp $
 *    $Revision: 1.1 $
...
// boards and will appear in all applications using this hardware
	...
	//detect number of PCRGB boards in the system		
	

	for (m_iBoardCount = 0;m_iBoardCount < MAX_BOARD_COUNT ;m_iBoardCount++) {

		if (!(m_pBrdMod[m_iBoardCount] = IfxCreateCaptureModule("BDT", m_iBoardCount,"bdttest.txt"))) {
			m_pBrdMod[m_iBoardCount] = IfxCreateCaptureModule("BDT", m_iBoardCount);
	...
	if(m_iBoardCount==0) {
		::MessageBox(NULL, "Can't find BanditII board", "Error", MB_OK | MB_ICONSTOP);
		return FALSE;
	}
