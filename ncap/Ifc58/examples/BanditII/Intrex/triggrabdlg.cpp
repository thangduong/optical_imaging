	...
	: CDialog(CTrigGrabDlg::IDD, pParent)
{
	...
	m_TrigEnable = (int) cam->GetAcqParam(P_TRIGGER_ENABLE);
	//}}AFX_DATA_INIT
...
void CTrigGrabDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_SW_TRIG, m_ctlTrigSrcSoft);
	DDX_Control(pDX, IDC_TTL_0, m_ctlTrigSrcExt);
	DDX_Radio(pDX, IDC_FALLING_EDGE, m_iTrigPolarity);
	DDX_Radio(pDX, IDC_SW_TRIG, m_iTrigSrc);
	DDX_Radio(pDX, IDC_TRG_ENABLE, m_TrigEnable);
	//}}AFX_DATA_MAP
}
...
BEGIN_MESSAGE_MAP(CTrigGrabDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_TRGAPPLY, OnTrgApply)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
BOOL CTrigGrabDlg::OnInitDialog() 
{
	...
	UpdateData(FALSE);
	if (!theApp->m_cam->AcqParamSupported(P_TRIGGER_SRC)) {
		m_ctlTrigSrcSoft.EnableWindow(FALSE);
		m_ctlTrigSrcExt.EnableWindow(FALSE);
	}
...
void CTrigGrabDlg::OnTrgApply() 
{
	...
	cam->SetAcqParam(P_TRIGGER_ENABLE, m_TrigEnable);	
}
