...
void CTrigGrabDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Radio(pDX, IDC_FRAME_POL_HI,		m_iFramePolarity);
...
void CTrigGrabDlg::RefreshDialogFromCamParams()
{
	...
	m_iStrobeDelay		= (int) cam->GetAcqParam(P_STROBE_DELAY); // lines post trigger
	...
	m_iFrameDelay		= (int) cam->GetAcqParam(P_FRAME_RESET_OFFSET);	 // lines post trigger
