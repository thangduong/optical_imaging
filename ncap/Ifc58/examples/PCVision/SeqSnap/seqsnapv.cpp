 ...
 *    $Logfile: /ifc/examples/PCVision/SEQSNAP/seqsnapv.cpp $
 *    $Revision: 1.1 $
 *    $Modtime: 4/22/02 10:23a $
 *
 * ABSTRACT
...
void CSeqsnapView::OnCmdsWritetiff() 
{
	...
	if(result == IDOK) {
		...
		IfxWriteImgFile(fname.GetBuffer(80),m_imgBuf,m_areaDx,m_areaDy,(WORD)attr.dwBitsPerPixel);
		fname.ReleaseBuffer();
...
{
	return ((AMV_NUM_LUT_ENTRIES - 1) - arg);
}

void CSeqsnapView::OnLutInvert() 
{
	CSeqsnapApp *theApp=(CSeqsnapApp*)AfxGetApp();	
	DWORD numEntries = AMV_NUM_LUT_ENTRIES;
...
void CSeqsnapView::OnLutThresh() 
{
	...
	CLutThrsh dg(this, AMV_NUM_LUT_ENTRIES-1);
...
void CSeqsnapView::OnCmdsHwzoom() 
{
	...
	if(dg.DoModal() == IDOK && (dg.m_xzoom != 1.0 || dg.m_yzoom != 1.0))
	{
		CAM_ATTR attr;
		...
		if((attr.dwWidth % (8 * (int) dg.m_xzoom)) == 0)
		{
			DelImageServer();
			InvalidateRect(NULL,TRUE);
