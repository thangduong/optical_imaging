 ...
 *    $Logfile: /ifc/examples/NetSightII/seqsnap/seqsnapv.cpp $
 *    $Revision: 1.1 $
...
BEGIN_MESSAGE_MAP(CSeqsnapView, CView)
	...
	ON_COMMAND(ID_CMDS_ADVANCEDIO, OnCmdsAdvancedIO)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
{
	return ((P2V_NUM_LUT_ENTRIES - 1) - arg);
}

void CSeqsnapView::OnLutInvert() 
{
	CSeqsnapApp *theApp=(CSeqsnapApp*)AfxGetApp();	
	DWORD numEntries = P2V_NUM_LUT_ENTRIES;
	theApp->m_cam->Freeze();
	theApp->m_cam->SetLutFunction(INPUT_LUT1, CSeqsnapView::LutInvert, &numEntries, LUT_INTELLI_WIDTH);
	ClearView();
	CreateImageServer();
}
...
void CSeqsnapView::OnLutLinear() 
{
	CSeqsnapApp *theApp = (CSeqsnapApp*) AfxGetApp();
	theApp->m_cam->Freeze();
	theApp->m_cam->LinearizeLut(INPUT_LUT1, LUT_INTELLI_WIDTH);
	ClearView();
	CreateImageServer();
}
...
void CSeqsnapView::OnLutThresh() 
{
	...
	CLutThrsh dg(this, P2V_NUM_LUT_ENTRIES-1);
	...
	if(dg.DoModal() == IDOK)
	{
		...
		theApp->m_cam->Freeze();
		theApp->m_cam->SetLutFunction(INPUT_LUT1, CSeqsnapView::LutFloor, &floor,LUT_INTELLI_WIDTH);
		ClearView();
		CreateImageServer();
	}
}
...
void CSeqsnapView::OnCmdsAdvancedIO() 
{
	CSeqsnapApp *theApp=(CSeqsnapApp*)AfxGetApp();	
	DelImageServer();  

	CAioThrd *trgThread = new CAioThrd(this);

	theApp->m_cam->DoParamGroupEditDialog( (IFC_PARAM_GROUP_TYPE)NSP_PGRP_AIO_SETTINGS, this);
