 ...
 *    $Logfile: /ifc/examples/NetSightII/seqsnap/seqsnap.cpp $
 *    $Revision: 1.1 $
...
void CSeqsnapApp::InitIFC(void)
{
	...
	if (!(m_iCap=IfxCreateCaptureModule("NSP",m_iActiveBoard, "nsptest.txt"))) {
	...
	m_iCap->OutportStrobeVal(IFC_LEVEL_HIGH);
	m_iCap->OutportVal(0xff);
	m_iCap->InportMode(FALSE, IFC_FALLING_EDGE);
	WORD parallelIO = m_iCap->InportVal();
