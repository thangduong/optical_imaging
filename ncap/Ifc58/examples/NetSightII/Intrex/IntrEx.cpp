 ...
 *    $Logfile: /ifc/examples/NetSightII/intrex/IntrEx.cpp $
 *    $Revision: 1.1 $
 *    $Modtime: 9/16/04 4:24p $
 *
 * ABSTRACT
...
BOOL CIntrExApp::InitInstance()
{
	...
	m_cam->SetAcqParam(	NSP_SYNC_SOURCE , NSP_SYNC_COMPOSITE_VIDEO);
...
// boards and will appear in all applications using this hardware
	...
	//detect number of NetSightII boards in the system		
	

	for (m_iBoardCount = 0;m_iBoardCount < MAX_BOARD_COUNT ;m_iBoardCount++) {

		if (!(m_pBrdMod[m_iBoardCount] = IfxCreateCaptureModule("NSP", m_iBoardCount,"nsptest.txt"))) {
			m_pBrdMod[m_iBoardCount] = IfxCreateCaptureModule("NSP", m_iBoardCount);
	...
	if(m_iBoardCount==0) {
		::MessageBox(NULL, "Can't find NetSightII board", "Error", MB_OK | MB_ICONSTOP);
		return FALSE;
	}
