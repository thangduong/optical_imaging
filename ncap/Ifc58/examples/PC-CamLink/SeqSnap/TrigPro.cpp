 ...
 *    $Logfile: /ifc/examples/CamLink/seqsnap/TrigPro.cpp $
 *    $Revision: 1.1 $
 *    $Modtime: 4/17/01 10:59a $
 *
 * ABSTRACT
...
: CPropertyPage(CTrigPro::IDD,TrigCaptions[TrigIndex])
{
	...
	m_GlitchDetectEnable = (int)cam->GetAcqParam(LNK_TRIG_GLITCH_DETECTOR,TrigIndex);
...
void CTrigPro::OnTApply() 
{
	...
	CICamera *cam=theApp->m_cam;
	UpdateData(TRUE);
	cam->SetAcqParam(LNK_TRIG_GLITCH_DETECTOR,m_GlitchDetectEnable,m_TrigIndex);
...
void CTrigPro::OnCancel() 
{
	CSeqsnapApp *theApp=(CSeqsnapApp*)AfxGetApp();	
	CICamera *cam=theApp->m_cam;
	m_GlitchDetectEnable = (int)cam->GetAcqParam(LNK_TRIG_GLITCH_DETECTOR,m_TrigIndex);
