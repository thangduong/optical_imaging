 ...
 *		$Logfile: /ifc/examples/CamLink/intrex/INTRDLG.CPP $
 *		$Revision: 1.1 $
 *		$Modtime: 12/01/03 2:28p $
 *
 * ABSTRACT  
...
void CIntrExDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_EDIT_SOFEN, m_lCounterGotSOFEN);
...
BEGIN_MESSAGE_MAP(CIntrExDlg, CDialog)
	...
	ON_WM_TIMER()
	ON_WM_CLOSE()
	ON_CONTROL_RANGE(BN_CLICKED, IDC_BUTTON1, IDC_BUTTON0B, OnButton)
	ON_CONTROL_RANGE(BN_CLICKED, IDC_BUTTON10, IDC_BUTTON1A, OnButton)
...
void CIntrExDlg::OnTimer(UINT nIDEvent) 
{
	...
	m_lCounterGotSOFEN = theApp->m_ulCounter[THREAD_SOFEN];
