 ...
 *		$Logfile: /ifc/examples/CamLink/intrex/IntrEx.cpp $
 *		$Revision: 1.1 $
 *		$Modtime: 5/09/01 2:39p $
 *
 * ABSTRACT  
...
// boards and will appear in all applications using this hardware
	...
	for (m_iBoardCount = 0;m_iBoardCount < MAX_BOARD_COUNT ;m_iBoardCount++) {
		...
		if (!(m_pBrdMod[m_iBoardCount] = IfxCreateCaptureModule("LNK", m_iBoardCount,"lnktest.txt"))) {
			m_pBrdMod[m_iBoardCount] = IfxCreateCaptureModule("LNK", m_iBoardCount);
