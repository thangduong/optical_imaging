 ...
 *		$Logfile: /ifc/examples/CamLink/intrex/TrgGrab.cpp $
 *		$Revision: 1.1 $
 *		$Modtime: 4/17/01 1:51p $
 *
 * ABSTRACT  
...
: CPropertyPage(CTrgGrab::IDD)
{
	...
	//}}AFX_DATA_INIT
	if (theApp->m_startEvent == LNK_TMODE_DISABLE)
		m_TrigStartEvent = 0;
	else
		m_TrigStartEvent = (DWORD)theApp->m_startEvent - 1;

	if (theApp->m_stopEvent == LNK_TMODE_DISABLE)
		m_TrigEndEvent = 0;
	else
...
void CTrgGrab::OnApplyTrig() 
{
	...
	if (m_TrigStartEvent != 0)
		theApp->m_startEvent = (LNK_TRIG_MODE_TYPE)(m_TrigStartEvent+1);
	else 
		theApp->m_startEvent = LNK_TMODE_DISABLE;

	if (m_TrigEndEvent != 0)
		theApp->m_stopEvent = (LNK_TRIG_MODE_TYPE)(m_TrigEndEvent+1);
	else 
		theApp->m_stopEvent = LNK_TMODE_DISABLE;

	((CPCLink*)(theApp->m_pActiveMod))->SetAcqTriggerEvents(theApp->m_startEvent ,theApp->m_stopEvent);
