 ...
 *    $Logfile: /ifc/examples/PCVisionplus/SeqSnap/TrgThrd.cpp $
 *    $Revision: 1.1 $
 *    $Modtime: 10/11/00 1:57p $
 *
 * ABSTRACT
...
DWORD WINAPI TriggerThread(LPVOID owner)
{
	...
	DWORD totalTime;
	BYTE *pHostBuffFrame;
	CInterrupt intr_eot(theapp->m_iCap, PCP_INTR_EOFRM);
	...
	j = m_areaDx / 2;
	if(attr.dwBitsPerPixel == 12){
		// In PCVisionPlus the default 12 bit data is right justified in a 16 bit pixel
		// So an optional flag IFC_IMGSRC_OPT_PIX_LEAST_SIG needs to be passed to IfxCreateImgSrc
		imgSrc = IfxCreateImgSrc(m_imgBuf,&attr,IFC_IMGSRC_OPT_PIX_LEAST_SIG);
		imgSink = IfxCreateImgSink(view->m_hWnd,DIB_SINK,0,0,(WORD)attr.dwWidth,	(WORD)attr.dwHeight);
		proc->m_imgConn = IfxCreateImgConn(imgSrc,imgSink);
	} else 
		proc->m_imgConn = IfxCreateImgConn(m_imgBuf,(WORD)attr.dwWidth,(WORD)attr.dwHeight,(WORD)attr.dwBitsPerPixel, view->m_hWnd);

	// Set board in mode: grab into host circular buffer.
	hGrabId = cam->Grab(0,m_imgBuf, FRAMES_IN_HOSTBUFF);
	Sleep(20);
	totalTime = 0;
	...
	if (m_trigProp->m_iTrigSrc == PCP_SOFT_TRIG){
	...
	for (frame_count=0;!proc->m_shutdown;) {
		...
		if (((iSeqNum=cam->GrabWaitFrameEx(hGrabId,&pHostBuffFrame,IFC_WAIT_NEWER_FRAME,500,TRUE,&AcquiredDy)) >= 0)
			...
			/*{
				deltaTime = singl_img_time - old_time;
				m_trigProp->m_maxTime = deltaTime;
				m_trigProp->m_minTime = deltaTime;
				m_trigProp->m_dwLastInterval = deltaTime;
			}*/
...
CVixnEOF::CVixnEOF(PCParentMod pMod) : CVixn(pMod,  PCP_INTR_EOFRM)
{
	m_intrCount = 0;
	m_deltaTime = 0;
	m_iSoftTrig = 1;
...
BOOL CVixnEOF::VixnMain()
{
	...
	if(m_iSoftTrig > 0)
	{
		WriteReg(PCP_SOFTTRIG_32, 1, VIXN_PARENT_MOD);
	}
