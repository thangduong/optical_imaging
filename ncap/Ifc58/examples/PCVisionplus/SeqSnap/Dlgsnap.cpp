 ...
 *    $Logfile: /ifc/examples/PCVisionplus/SeqSnap/Dlgsnap.cpp $
 *    $Revision: 1.1 $
 *    $Modtime: 6/17/03 3:05p $
 *
 * ABSTRACT
...
BOOL CDlgsnap::OnInitDialog() 
{
	...
	if(attr.dwBitsPerPixel == 12){
		m_ImgSrc = IfxCreateImgSrc(m_imgBuf,&attr,IFC_IMGSRC_OPT_PIX_LEAST_SIG);
		m_ImgSink = IfxCreateImgSink(m_vparent->m_hWnd,DIB_SINK,0,0,(WORD)m_areaDx,	(WORD)m_areaDy);
		m_ImgConn = IfxCreateImgConn(m_ImgSrc,m_ImgSink);
	} else 
		m_ImgConn = IfxCreateImgConn(m_imgBuf,(WORD)m_areaDx,(WORD)m_areaDy,(WORD)attr.dwBitsPerPixel, m_vparent->m_hWnd);
...
void CDlgsnap::OnSave() 
{
	...
	//	else {			
			for(int i=0;i< m_vparent->num_snap_areas;i++){
				...
				CString sNum;
				sNum.Format("%d", i);
				CImgFile *imgFile=IfxCreateImgFile((fname +sNum + "." + dg.GetFileExt()).GetBuffer(80));
				if (imgFile) {
					IFFCLASS_TYPE File; //FileColor;
					...
					if (!imgFile->WriteFile(m_imgBuf + i * (m_areaDx*m_areaDy*m_bytesPerPixel),m_areaDx,m_areaDy,(WORD)attr.dwBitsPerPixel,File))
...
theApp->m_cam->GetAttr(&attr);
	...
	if(result == IDOK)
	{
		...
		save_to_AVI_file(m_NumFrames, 0, FName.GetBuffer(FName.GetLength()), m_imgBuf, attr.dwWidth, attr.dwHeight, attr.dwBitsPerPixel, 30L,0,  0,  0, 0, 0 );
	}
}
