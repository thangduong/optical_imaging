 ...
 *    $Logfile: /ifc/examples/PCVisionplus/IntrEx/INTREX.CPP $
 *    $Revision: 1.1 $
 *    $Modtime: 8/23/00 12:43p $
 *
 * ABSTRACT
...
// boards and will appear in all applications using this hardware
	...
	for (m_iBoardCount = 0;m_iBoardCount < MAX_BOARD_COUNT ;m_iBoardCount++) {
		...
		if (!(m_pBrdMod[m_iBoardCount] = IfxCreateCaptureModule("PCP", m_iBoardCount,"pcptest.txt"))) {
			m_pBrdMod[m_iBoardCount] = IfxCreateCaptureModule("PCP", m_iBoardCount);
