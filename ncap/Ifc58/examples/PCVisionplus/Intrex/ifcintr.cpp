 ...
 *    $Logfile: /ifc/examples/PCVisionplus/IntrEx/ifcintr.cpp $
 *    $Revision: 1.1 $
 *    $Modtime: 8/24/00 10:35a $
 *
 * ABSTRACT
...
UINT IntrThreadProc(LPVOID pParam)
{
	...
	CIntrExDlg* dlg = (CIntrExDlg*) theApp->m_pDlg ;
	DWORD CurrSysTime = 0, LastSysTime, DeltaTime, LastTime, TotalTime = 0, MaxTime = 0, AvgTime,event = pParent->GetEvent();
	enum{LAST_TIME,
		MAX_TIME,
		AVG_TIME
	};
	...
	while (pParent->m_bContinue == TRUE) {
		...
		// Time Statistics
			...
			CurrSysTime = pParent->m_pMod->m_os->GetSystimeMillisecs();
				if(!(pParent->m_IsResumed))
				{
					DeltaTime = CurrSysTime - LastSysTime;
					LastTime = DeltaTime;
					if((theApp->m_ulCounter[event]) == 1) // user resetted the counters
					{
						TotalTime = 0;
						MaxTime = 0;
					}
					if (DeltaTime > MaxTime)
						MaxTime = DeltaTime; 
					TotalTime += DeltaTime;
					if ((theApp->m_ulCounter[event]) > 0)
						AvgTime = (DWORD) TotalTime / theApp->m_ulCounter[event];
					dlg->m_TimeStat[event + 11 * LAST_TIME] = LastTime;
					dlg->m_TimeStat[event + 11 * MAX_TIME] = MaxTime;
					dlg->m_TimeStat[event + 11 * AVG_TIME] = AvgTime;
				}else
					pParent->m_IsResumed = FALSE;
			//}
...
IFC_INTREVENT CIfcInterrupt::GetEvent()
{
	return m_iEvent;
}
