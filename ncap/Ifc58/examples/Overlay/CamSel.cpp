 ...
 *    $Logfile: /ifc/examples/overlay/CamSel.cpp $
 *    $Revision: 1.1 $
 *    $Modtime: 5/07/99 2:22p $
 *
 * ABSTRACT
...
void CCamSel::OnSelchangeListCamname() 
{
	...
	m_parent->m_cam->SetCameraType(curCamera);
}
...
BOOL CCamSel::OnInitDialog() 
{
	...
	for(camname = m_parent->m_cam->GetFirstCamType();camname;camname = m_parent->m_cam->GetNextCamType()) {
	...
	CAM_ATTR camAttr;
	m_parent->m_cam->GetAttr(&camAttr);
