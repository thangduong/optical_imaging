 ...
 *    $Logfile: /ifc/examples/PC2-Vision/IntrEx/IntrEx.cpp $
 *    $Revision: 1.1 $
 *    $Modtime: 4/06/04 10:36a $
 *
 * ABSTRACT
...
// boards and will appear in all applications using this hardware
	...
	//detect number of PCVision2 boards in the system		
	

	for (m_iBoardCount = 0;m_iBoardCount < MAX_BOARD_COUNT ;m_iBoardCount++) {

		if (!(m_pBrdMod[m_iBoardCount] = IfxCreateCaptureModule("P2V", m_iBoardCount,"p2vtest.txt"))) {
			m_pBrdMod[m_iBoardCount] = IfxCreateCaptureModule("P2V", m_iBoardCount);
	...
	if(m_iBoardCount==0) {
		::MessageBox(NULL, "Can't find PC2Vision board", "Error", MB_OK | MB_ICONSTOP);
		return FALSE;
	}
