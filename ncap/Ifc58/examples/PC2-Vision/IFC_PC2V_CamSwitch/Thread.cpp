...
//              Constructor
//------------------------------------------
CThread::CThread(CString strThreadName)
{
    ...
    m_pEvtWakeUpThread = new CEvent(FALSE, TRUE, NULL, NULL);
    ...
    // Keep my name in case error on destruction

    m_strMyName = strThreadName;
...
CString strToShow;
    ...
    if (dwRetry == 0)
    {
        if (m_strMyName.GetLength())
        {
            strToShow = _T("The thread ");
            strToShow += m_strMyName;
            strToShow += _T(" did not terminate normally !!!");
            AfxMessageBox(strToShow);
        }
        else
        {
