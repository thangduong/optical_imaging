 ...
 *    $Logfile: /ifc/examples/LineScan/seqsnap/seqsnap.cpp $
 *    $Revision: 1.1 $
 *    $Modtime: 6/29/00 3:58p $
 *
 * ABSTRACT
...
void CSeqsnapApp::InitIFC(void)
{
	...
	if (!(m_iCap=(CPCLine*)IfxCreateCaptureModule("PCL",m_iActiveBoard,"pcltest.txt"))) {
		if (!(m_iCap=(CPCLine*)IfxCreateCaptureModule("PCL",m_iActiveBoard))) {
	...
	// entire buffer is too large to be locked at once, Note this must be done before any Grab operation is attempted
	m_iCap->EnableJitGrabMemLock();

	m_cam = (CPCLCam*)m_iCap->GetCam(0);
	CAM_ATTR attr;
