 ...
 *		$Logfile: /ifc/examples/LineScan/IntrEx/IntrEx.cpp $
 *		$Revision: 1.1 $
 *		$Modtime: 5/18/00 1:53p $
 *
 * ABSTRACT  
...
BOOL CIntrExApp::InitInstance()
{
	...
	// clean-up IFC
	for(int n=0; n < PCL_INTR_MAX_EVENT;n++) 
		delete m_pIntrObj[n];
...
// boards and will appear in all applications using this hardware
	...
	//detect number of PCLIG boards in the system		
	

	for (m_iBoardCount = 0;m_iBoardCount < MAX_BOARD_COUNT ;m_iBoardCount++) {

		if (!(m_pBrdMod[m_iBoardCount] = IfxCreateCaptureModule("PCL", m_iBoardCount,"pcltest.txt"))) {
			m_pBrdMod[m_iBoardCount] = IfxCreateCaptureModule("PCL", m_iBoardCount);
...
INTERRUPTPROCARGS InterruptProcArgs[PCL_INTR_MAX_EVENT];
