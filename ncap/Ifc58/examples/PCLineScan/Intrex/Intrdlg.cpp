 ...
 *		$Logfile: /ifc/examples/LineScan/IntrEx/INTRDLG.CPP $
 *		$Revision: 1.1 $
 *		$Modtime: 5/18/00 1:52p $
 *
 * ABSTRACT  
...
void CIntrExDlg::OnTimer(UINT nIDEvent) 
{
	...
	// update the counters
	m_lCounterGotVB= theApp->m_ulCounter[PCL_INTR_VB];
	m_lCounterGotHB = theApp->m_ulCounter[PCL_INTR_HB];
	m_lCounterGotTRIG0 = theApp->m_ulCounter[PCL_INTR_TRIG0];
	m_lCounterGotTRIG1 = theApp->m_ulCounter[PCL_INTR_TRIG1];
	m_lCounterGotSOACQ = theApp->m_ulCounter[PCL_INTR_SOACQ];
	m_lCounterGotEOACQ = theApp->m_ulCounter[PCL_INTR_EOACQ];
	m_lCounterGotINPORT = theApp->m_ulCounter[PCL_INTR_INPORT];
	m_lCounterGotACQ_ADRS = theApp->m_ulCounter[PCL_INTR_ACQ_ADRS];
	m_lCounterGotBMDONE = theApp->m_ulCounter[PCL_INTR_BMDONE];
	m_lCounterGotSW_TIMER = theApp->m_ulCounter[PCL_INTR_SW_TIMER];
...
void CIntrExDlg::OnReset() 
{
	...
	for(int i=0; i < PCL_INTR_MAX_EVENT;i++)
