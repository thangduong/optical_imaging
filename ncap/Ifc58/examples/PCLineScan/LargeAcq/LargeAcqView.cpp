 ...
 *    $Logfile: /ifc/examples/LineScan/LargeAcq/LargeAcqView.cpp $
 *    $Revision: 1.1 $
 *    $Modtime: 6/12/00 11:08a $
 *
 * ABSTRACT
 *    PCLine Large Frame Acquisition Example Program
 *	This program shows how the PCLine can be used to acquire very large frames.
...
BEGIN_MESSAGE_MAP(CLargeAcqView, CScrollView)
	...
	ON_COMMAND(ID_FLATFIELD, OnFlatField)
	//}}AFX_MSG_MAP
...
void CLargeAcqView::OnFlatField() 
{
	CLargeAcqApp *theapp=(CLargeAcqApp*)AfxGetApp();	
	StopGrab();
	theapp->m_cam->Grab();

	// Must be in non-hot memory grab state when call FlatFieldSettingsDlg
	{
		// Make a direct camera display image connection for live display during FlatFieldSettingsDlg
		CImgConn ImgConn(theapp->m_cam, m_hWnd,IFC_LIVE_IMAGE ,ICAP_INTR_VB);

		((CPCLCam*)theapp->m_cam)->FlatFieldSettingsDlg();	
	}
}
