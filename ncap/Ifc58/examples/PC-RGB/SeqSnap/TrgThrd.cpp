 ...
 *    $Logfile: /ifc/examples/PcRgb/SEQSNAP/TrgThrd.cpp $
 *    $Revision: 1.1 $
 *    $Modtime: 7/16/02 10:11a $
 *
 * ABSTRACT
...
DWORD WINAPI TriggerThread(LPVOID owner)
{
	...
	DWORD totalTime;
	BYTE *pHostBuffFrame;
	CInterrupt intr_eot(theapp->m_iCap, PCR_INTR_EOFRM);
	...
	for (frame_count=0;!proc->m_shutdown;) {
		if (iSeqNum < 0 && m_trigProp->m_iTrigSrc == PCR_SOFT_TRIG)
		...
		if (((iSeqNum=cam->GrabWaitFrameEx(grabID,&pHostBuffFrame,IFC_WAIT_NEWER_FRAME,500,TRUE,&AcquiredDy)) >= 0)
			...
			if (m_trigProp->m_iTrigSrc == PCR_SOFT_TRIG)
				cam->SetAcqParam(P_GEN_SW_TRIGGER, 0);
