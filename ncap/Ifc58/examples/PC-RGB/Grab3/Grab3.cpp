 ...
 *    $Logfile: /ifc/examples/PcRgb/Grab3/Grab3.cpp $
 *    $Revision: 1.1 $
 *    $Modtime: 2/11/00 10:52a $
 *
 * ABSTRACT
...
BOOL CGrab3App::IFCInit()
{
	...
	if (!(m_pMod=IfxCreateCaptureModule("PCR",0,"pcrtest.txt"))) {
		if (!(m_pMod=IfxCreateCaptureModule("PCR",0,NULL))) {
	...
	if (m_camAttrib.color != IFC_RGB_PLANAR	&& m_camAttrib.color != IFC_aRGB_PLANAR) {
		m_cam->SetAcqParam(P_PIXEL_COLOR,IFC_RGB_PLANAR);
		m_cam->SetAcqParam(P_WIDTH_PIXELS,m_camAttrib.dwWidth*3);
		m_cam->GetAttr(&m_camAttrib);
	}
