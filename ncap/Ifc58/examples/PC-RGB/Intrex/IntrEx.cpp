 ...
 *		$Logfile: /ifc/examples/PcRgb/IntrEx/IntrEx.cpp $
 *		$Revision: 1.1 $
 *		$Modtime: 10/27/99 3:56p $
 *
 * ABSTRACT  
...
// boards and will appear in all applications using this hardware
	...
	for (m_iBoardCount = 0;m_iBoardCount < MAX_BOARD_COUNT ;m_iBoardCount++) {
		...
		if (!(m_pBrdMod[m_iBoardCount] = IfxCreateCaptureModule("PCR", m_iBoardCount,"pcrtest.txt"))) {
			m_pBrdMod[m_iBoardCount] = IfxCreateCaptureModule("PCR", m_iBoardCount);
