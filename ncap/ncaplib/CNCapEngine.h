...
//	char m_pSnapBaseDir[512];
	...
	int m_iFilterWheelTriggerBit;
	int m_iFilterState;
	int numFramesPerWavelength; // for multi-spectral imaging

	void StreamOnLPT(const char* filename, int bitNum);
	void MSStreamOnLPT(const char* filename, int bitNum);
...
public:
	...
	virtual HRESULT __stdcall SetShutterState(int state);
	virtual HRESULT __stdcall GetShutterState(int* state);
	virtual HRESULT __stdcall GoFilter(bool updatePos = true);	
	virtual HRESULT __stdcall StopFilter();
	virtual HRESULT __stdcall GetFilterState(int* fs) {
		(*fs) = m_iFilterState; return S_OK;
	}
	virtual HRESULT __stdcall SetFilterState(int fs);
	virtual HRESULT __stdcall GetFramesPerWavelength(int* fpw) {
		(*fpw) = numFramesPerWavelength; return S_OK;
	}
	virtual HRESULT __stdcall SetFramesPerWavelength(int fpw) {
		numFramesPerWavelength = fpw; return S_OK;
	}

	virtual HRESULT __stdcall StartStreamOnLPT(const unsigned char* filename, int bitNum);
	virtual HRESULT __stdcall StartMSStreamOnLPT(const unsigned char* filename, int bitNum);
