...
for rep=1:2
    for cond=1:ncond
        ...
        if (length(paradigm{rep}{cond}.varargin)>11)
            ...
            if ((length(bpp)>0)&(length(enum)>0))
                cydata{enum}{bpp} = addmovie(cydata{enum}{bpp},curData.frames);
                cydata{enum}{bpp} = cydata{enum}{bpp} + 1; 
            end
        end
