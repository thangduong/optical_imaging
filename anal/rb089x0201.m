...
t = 1:(6*(1+3+5));
tf = [];
or = [-22.5 -45 -67.5 -90 -112.5 -135 -157.5 -180];
sf = [.1 .16 .26 .43 0.7 1.13 1.84];
bphase = [];
eye = [0 1];
sfdata = {};
sfcount = {};
...
for rep=1:1
    for cond=1:ncond
        if (paradigm{rep}{cond}.varargin{5}~=1)
            % blank condition
            continue;
        end
        ...
        if (length(paradigm{rep}{cond}.varargin)>2)
            bpp = find(paradigm{rep}{cond}.varargin{3} == sf);
            if (length(bpp)>0)
                sfdata{bpp} = addmovie(sfdata{bpp},curData.frames);
                sfcount{bpp} = sfcount{bpp} + 1; 
            end
        end
