...
for rep=1:nrep
    for cond=1:ncond
        ...
        if (length(paradigm{rep}{cond}.varargin)>11)
            ...
            onum = find(paradigm{rep}{cond}.varargin{2} == or);
            if ((length(bpp)>0)&(length(enum)>0)&(length(onum)>0))
                cydata{onum}{enum}{bpp} = addmovie(cydata{onum}{enum}{bpp},curData.frames);
                cycount{onum}{enum}{bpp} = cycount{onum}{enum}{bpp} + 1; 
            end
        end
