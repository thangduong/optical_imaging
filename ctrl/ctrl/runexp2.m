% RUNEXP2    Runs an optical imaging experiment (with hacked wait for dye)
%
...
%   runexp('mseq', 5);
%
function runexp(protocol, nreps, waitsec, varargin);
...
for rep = 1:nreps
    ...
    if (rep ~= nreps)
        fprintf(1,'waiting rest time\r\n');
        for waitcur = 1:ceil(waitsec)
            pause(1);
            fprintf(1,'.');
        end
        disp('done!');
